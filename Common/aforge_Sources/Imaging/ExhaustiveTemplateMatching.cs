...
namespace AForge.Imaging
{
    ...
    public class ExhaustiveTemplateMatching : ITemplateMatching
    {
        ...
        /// <param name="iStepX"></param>
        /// <param name="iHeight"></param>
        /// <param name="DifMax">每个像素，最大差值</param>
        /// <returns></returns>
        public List<TemplateMatch> ProcessImage2(Bitmap image, Bitmap template, int iStepX,int iHeight,int DifMax)
        {
            // lock source and template images
            BitmapData imageData = image.LockBits(
                new Rectangle(0, 0, image.Width, image.Height),
            ...
            return ProcessImage2(new UnmanagedImage(imageData), new UnmanagedImage(templateData), iStepX,iHeight, DifMax);
        }
        ...
        /// <param name="image">Unmanaged source image to process.</param>
        /// <param name="template">Unmanaged template image to search for.</param>
        /// <param name="iStepX">步长</param>
        /// <param name="iHeight">比较的高度，超过就不比较了</param>
        /// <param name="DifMax">每个像素，最大差值</param>
        /// 
        /// <returns>Returns array of found template matches. The array is sorted by similarity
        /// of found matches in descending order.</returns>
        ...
        /// <exception cref="InvalidImagePropertiesException">Template image is bigger than search zone.</exception>
        ///
        public List<TemplateMatch> ProcessImage2(
            UnmanagedImage image, UnmanagedImage template,
            int iStepX,int iHeight, int DifMax)
        {
            // check image format
            /*
             * if (
            ...
            */
            ...
            int pixelSize = 3;// (image.PixelFormat == PixelFormat.Format8bppIndexed) ? 1 : 3;
            int sourceStride = image.Stride;
            ...
            // maximum possible difference with template
            int maxDiff = templateWidth / iStepX * iHeight * pixelSize * 255 ;
            ...
            unsafe
            {
                ...
                for (int y = 0; y < mapHeight; y++)
                {
                    ...
                    for (int x = 0; x < mapWidth; x++)
                    {
                        ...
                        // for each row of the template
                        for (int i = 0; i < Math.Min( templateHeight, iHeight); i++)
                        {
                            // for each pixel of the template
                            int j = 0;
                            for (j = 0; j < templateWidthInBytes - iStepX; j+= iStepX, src+= iStepX, tpl+= iStepX)
                            {
                                int d;// = *src - *tpl;
                                d= Math.Abs(*src - *tpl)+ Math.Abs(*(src+1) - *(tpl+1))+ Math.Abs(*(src+2) - *(tpl+2));
                                if (d > DifMax)
                                {
                                    bError = true;
                                    break;
            ...
            */
            ...
            return matchingsList;// matchings;
        }
        ...
        /// <returns></returns>
        public List<TemplateMatch> ProcessImage3(Bitmap image, Bitmap template, List<TemplateMatch> pList)
        {
            ...
            BitmapData templateData = template.LockBits(
                new Rectangle(0, 0, template.Width, template.Height),
                ImageLockMode.ReadOnly, template.PixelFormat);
            return ProcessImage3(new UnmanagedImage(imageData), new UnmanagedImage(templateData), pList);
        }

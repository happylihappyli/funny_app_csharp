...
Namespace Funny
    Public Module S_China
        'ºº×Ö¼ò·±×ª»»aspº¯Êý/asp¼òÌå×ª·±Ìå(¼ò·±×ª»»)
        Public Enum FunnyConvertType
            Fan_Jian = 0
            Jian_Fan = 1
        End Enum

        Function Convert_FanJian(ByVal str As String, ByVal eType As FunnyConvertType) As String
            Dim strReturn As String = ""
            'surnfujtofstr = ""
            If eType = FunnyConvertType.Jian_Fan Then ' @#¼òµ½·³
                strReturn = Strings.StrConv(str, VbStrConv.TraditionalChinese, 0)
                'surnfujtofstr = surnfujof(str, surnfujian, surnfufan)
            ElseIf eType = FunnyConvertType.Fan_Jian Then  '@#¼òµ½·³
                strReturn = Strings.StrConv(str, VbStrConv.SimplifiedChinese, 0)
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Function PinYin(ByVal Hz As String)
            Dim strReturn As String = ""
            Dim PinMa As String
                ...
                If Temp < 0 Then
                    ...
                    For j = 791 To 1 Step -2
                        If Temp <= Val(MyPinMa(j)) Then
                            strReturn = strReturn & MyPinMa(j - 1) & ""
                            Exit For
                        End If
                    Next
                End If
